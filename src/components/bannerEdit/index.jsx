    ...
    return (
        <div className={classes.editBanner}>
            ...
            <div className={classes.choseForm}>
                <input type="file" onChange={convertTo64Base} />
                <span>Выберете файл</span>
            </div>
            <input type='text' className={classes.input} placeholder='Введите полную ссылку' value={link} onChange={e=>setLink(e.target.value)} />
        <div>
        <button className={classes.addBannerButton} onClick={()=>{postOrPut()}} />
        <button className={classes.deleteBannerButton} onClick={()=>{del()}} />                         
        </div>
    </div>
    );
};
